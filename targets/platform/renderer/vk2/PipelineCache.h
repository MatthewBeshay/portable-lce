...
/// Pipeline state key — one pipeline per unique state combination.
/// Matches bgfx's approach: all render state baked into the pipeline,
/// dynamic state limited to viewport, scissor, blend constants.
struct PsoKey {
...
/// Creates and caches VkPipeline objects by PsoKey.
class PipelineCache {
public:
    struct Config {
